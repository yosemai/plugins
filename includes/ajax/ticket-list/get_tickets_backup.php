...
$ticket_filter = isset($_POST['filter']) ? $_POST['filter'] : array();
if( !$ticket_filter ){
    die(__('Filter not found!', 'wp-support-plus-responsive-ticket-system-emancipatic'));
}

@setcookie("wpsp_ticket_filters", base64_encode(serialize($ticket_filter)), 0, COOKIEPATH);
...
<div class="row">
    ...
    <div class="col-md-9" style="text-align: left;">
        <button type="button" class="btn btn-sm btn-default wpsp_btn_ticket_list_action visible-sm visible-xs" id="btn_mo_reset_filter" onclick="btn_reset_ticket_filter(this);"><?php _e('Reset Filter','wp-support-plus-responsive-ticket-system-emancipatic')?></button>
        <?php if($default_filters['ticket_list_filter']==1) {?>
								<button type="button" onclick="get_ticket_filter();"class="btn btn-sm btn-default wpsp_btn_ticket_list_action" id="btn_ticket_filter"><?php _e('Hide Filters','wp-support-plus-responsive-ticket-system-emancipatic')?></button>
				<?php
			} else{ ?>
								<button type="button" onclick="get_ticket_filter();"class="btn btn-sm btn-default wpsp_btn_ticket_list_action" id="btn_ticket_filter"><?php _e('Show Filters','wp-support-plus-responsive-ticket-system-emancipatic')?></button>
			<?php
			}
        if( $wpsupportplus->functions->cu_has_cap( 'bulk_change_status' ) ){
            ?>
            <button type="button" onclick="get_bulk_change_status('<?php echo wp_create_nonce()?>');" class="btn btn-sm btn-default wpsp_btn_ticket_list_action checkbox_depend disabled" id="btn_change_status"><?php _e('Change Status','wp-support-plus-responsive-ticket-system-emancipatic')?></button>
            <?php
        }
        ?>
        <?php
         if( $wpsupportplus->functions->cu_has_cap( 'bulk_assign_agent' ) ){
            ?>
            <button type="button" onclick="get_bulk_assign_agent('<?php echo wp_create_nonce()?>');"class="btn btn-sm btn-default wpsp_btn_ticket_list_action checkbox_depend disabled" id="btn_assign_agent"><?php _e('Assign Agent','wp-support-plus-responsive-ticket-system-emancipatic')?></button>
            <?php
        }
        ?>
        <?php
        if( $wpsupportplus->functions->cu_has_cap( 'bulk_delete_ticket' ) ){
            ?>
            <button type="button" onclick="get_delete_bulk_ticket('<?php echo wp_create_nonce()?>');"class="btn btn-sm btn-default wpsp_btn_ticket_list_action checkbox_depend disabled" id="btn_delete_ticket"><?php _e('Delete','wp-support-plus-responsive-ticket-system-emancipatic')?></button>
            <?php
        }
        ?>
    ...
    <div class="col-md-3" style="text-align: right;">
        ...
        if( $total_items > $per_page ){
            printf(__('<strong>%d</strong>-<strong>%d</strong> of <strong>%d</strong> Tickets','wp-support-plus-responsive-ticket-system-emancipatic'),(($current_page*$per_page-$per_page)+1),($current_page*$per_page),$total_items);
        } else {
            printf(__('<strong>%d</strong> Tickets','wp-support-plus-responsive-ticket-system-emancipatic'),$total_items);
        }
        ?>
    ...
    <table id="tbl_wpsp_ticket_list" class="table">
        ...
        <tbody>
            ...
            foreach ( $tickets as $ticket ){
                ...
                ?>
                <tr data-href="<?php echo $wpsupportplus->functions->get_support_page_url(array('page'=>'tickets','section'=>'ticket-list','action'=>'open-ticket','id'=>$ticket->ID))?>" onclick="if(link)wpsp_redirect(this);">
                    <?php
                    if($wpsupportplus->functions->is_staff($current_user)){
                        ?>
            ...
            if( !$total_items ){
                ?>
                <tr>
                    <td colspan="<?php echo count($list_keys)+1?>" style="text-align: center;"><?php _e('No Tickets found!','wp-support-plus-responsive-ticket-system-emancipatic')?></td>
                </tr>
                <?php
            }
            ?>
...
<?php if($total_items) :?>
    <div class="row">
        <div class="col-md-4 col-md-offset-4" style="text-align: center;">
            ...
            <?php
            printf(__('<strong>%d</strong> of <strong>%d</strong> Pages','wp-support-plus-responsive-ticket-system-emancipatic'), $current_page, $total_pages);
            ?>
            <button class="btn btn-default btn-sm" onclick="wpsp_ticket_next_page();"><i class="fa fa-chevron-circle-right" aria-hidden="true"></i></button>
...
$(function () {
    jQuery('.wpsp_header_sort').click(function(){

        var key     = jQuery(this).attr('data-orderKey');
        var state   = parseInt(jQuery(this).attr('data-orderState'));
        var order   = jQuery(this).attr('data-orderbyOrder');
        ...
        jQuery('#wpsp_filter_sort_by').val(key);
        jQuery('#wpsp_filter_sort_by_order').val(order);
